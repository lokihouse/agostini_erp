...
class TransportOrderResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Tabs::make('Transport Order Details')
                    ->tabs([
                        Tabs\Tab::make('Informações da Ordem')
                            ->schema([
                                // Envolvemos o conteúdo da aba em um Group para aplicar o disabled condicionalmente
                                    ...
                                    ->schema([
                                        Forms\Components\Grid::make(2)->schema([
                                            Forms\Components\TextInput::make('transport_order_number')
                                                ->label('Número da OT')
                                                ->disabled() // Já estava, mas é bom manter
                        ...
                        Tabs\Tab::make('Cancelamento')
                            ->visible(fn (callable $get) => $get('status') === TransportOrder::STATUS_CANCELLED)
                            ->schema([
                                // Se uma ordem concluída pudesse ser cancelada (improvável), estes campos também precisariam do disabled
                                // Mas a visibilidade da aba já trata isso para o status COMPLETED.
                        ...
                        Tabs\Tab::make('Observações Gerais')
                            ->schema([
                                Forms\Components\Group::make()
                                    ->disabled(fn (?Model $record): bool => $record instanceof TransportOrder && $record->status === TransportOrder::STATUS_COMPLETED)
                                    ->schema([
