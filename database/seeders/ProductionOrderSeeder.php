...
use App\Models\ProductionStep; use App\Models\User; use Illuminate\Database\Console\Seeds\WithoutModelEvents;
        ...
        $this->command->info('Criando Ordens de Produção, Itens e Logs...');
            ...
            ->for($company)             ->for($user)                ->has(
                ProductionOrderItem::factory()
                    ->count(rand(1, 3))
                    ->for($company)                     ->state(function (array $attributes, ProductionOrder $order) use ($products, $productionSteps) {                                                 $product = $products->whereNotIn('uuid', $order->items->pluck('product_uuid'))->random();
                    ...
                    ->afterCreating(function (ProductionOrderItem $item) use ($productionSteps, $company, $user) {                         if ($item->quantity_planned <= 0 || $productionSteps->isEmpty()) return;
