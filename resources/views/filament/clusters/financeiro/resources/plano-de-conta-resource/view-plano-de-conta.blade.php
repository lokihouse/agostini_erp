    ...
    @if(count($tabs))
        <section class="">
            <table class="w-full rounded-xl bg-gray-200 shadow-sm dark:bg-gray-900 dark:ring-white/10">
                ...
                @foreach($tabs as $tab)
                    <tr  class="font-extrabold bg-gray-300 cursor-pointer" x-on:click="window.location.href = '{{ route('filament.app.financeiro.resources.plano-de-contas.edit', [$tab['id']]) }}'">
                        ...
                        <td class="text-right pr-4 border-right border-gray-200">
                            @if($tab['valor_realizado'] >= 0)
                                {{ \App\Utils\TextFormater::toMoney($tab['valor_realizado'] ?? 0) }}
                            @else
                                @php
                                    $val = floatval($tab['valor_realizado'] ?? 0) * -1;
                                @endphp
                                <span class="text-red-500">({{ \App\Utils\TextFormater::toMoney($val) }})</span>
                            @endif
                        </td>
                        <td class="pr-4 text-right">
                            @if($tab['valor_projetado'] >= 0)
                                {{ \App\Utils\TextFormater::toMoney($tab['valor_projetado'] ?? 0) }}
                            @else
                                @php
                    ...
                    @foreach($contas_1 as $tab_1)
                        <tr  class="bg-gray-200 font-medium  cursor-pointer" x-on:click="window.location.href = '{{ route('filament.app.financeiro.resources.plano-de-contas.edit', [$tab_1['id']]) }}'">
                            ...
                            <td class="text-right pr-4 border-right border-gray-200">
                                @if($tab_1['valor_realizado'] >= 0)
                                    {{ \App\Utils\TextFormater::toMoney($tab_1['valor_realizado'] ?? 0) }}
                                @else
                                    @php
                            ...
                            <td class="pr-4 text-right">
                                @if($tab_1['valor_projetado'] >= 0)
                                    {{ \App\Utils\TextFormater::toMoney($tab_1['valor_projetado'] ?? 0) }}
                                @else
                                    @php
                        ...
                        @foreach($contas_2 as $tab_2)
                            <tr  class="bg-gray-200 font-light  cursor-pointer" x-on:click="window.location.href = '{{ route('filament.app.financeiro.resources.plano-de-contas.edit', [$tab_2['id']]) }}'">
                                ...
                                <td class="text-right pr-4 border-right border-gray-200">
                                    @if($tab_2['valor_realizado'] >= 0)
                                        {{ \App\Utils\TextFormater::toMoney($tab_2['valor_realizado'] ?? 0) }}
                                    @else
                                        @php
                                ...
                                <td class="pr-4 text-right">
                                    @if($tab_2['valor_projetado'] >= 0)
                                        {{ \App\Utils\TextFormater::toMoney($tab_2['valor_projetado'] ?? 0) }}
                                    @else
                                        @php
                            ...
                            @foreach($contas_3 as $tab_3)
                                <tr  class="bg-gray-200 font-light  cursor-pointer" x-on:click="window.location.href = '{{ route('filament.app.financeiro.resources.plano-de-contas.edit', [$tab_3['id']]) }}'">
                                    ...
                                    <td class="text-right pr-4 border-right border-gray-200">
                                        @if($tab_3['valor_realizado'] >= 0)
                                            {{ \App\Utils\TextFormater::toMoney($tab_3['valor_realizado'] ?? 0) }}
                                        @else
                                            @php
                                    ...
                                    <td class="pr-4 text-right">
                                        @if($tab_3['valor_projetado'] >= 0)
                                            {{ \App\Utils\TextFormater::toMoney($tab_3['valor_projetado'] ?? 0) }}
                                        @else
                                            @php
